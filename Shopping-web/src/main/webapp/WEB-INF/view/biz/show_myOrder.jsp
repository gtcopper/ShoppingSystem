    ...
    <div class="wrap">
        ...
        <c:forEach items="${sessionScope.orders}" var="order">
            <div style="border:1px solid #000;">
                    <p style="float: left;margin-left: 20px; margin-right: 20px;margin-top: 60px;">订单号: ${order.orderNumber}</p>
                    <p class="price" style="float: left;margin-left: 20px; margin-right: 10px;margin-top: 60px;">订单价格:      <span class="priceDetail">${order.orderPrice}</span>元</p>
                    <p style="float: left;margin-left: 20px;margin-top: 60px;">订单状态 :
                        <c:if test="${order.payStatus == 1}">已支付</c:if>
                        <c:if test="${order.payStatus == 0}">未支付</c:if>
                    </p>
                    <p style="float: left;margin-left: 20px;margin-top: 60px;">订单创建时间 : <fmt:formatDate value="${order.orderCreateTime}" pattern="yyyy/MM/dd hh:mm:ss aa" /></p>
                    <p style="float: left;margin-left: 20px;margin-top: 60px;"><a href="order/orderDetails/${order.orderId}" target="_blank">查看订单详情</a></p>
                    <form action="order/deleteOrder" method="post">
                        <input type="hidden" name="orderId" value="${order.orderId}"/>
                        <button class="btnSubmit grey2" style="margin-top: 60px;" type="button">删除订单</button>
                    </form>
                <div class="clear"></div>
            </div>
        </c:forEach>
